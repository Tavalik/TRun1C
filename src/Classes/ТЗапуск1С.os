...
Функция COM_УдалитьИнформационнуюБазу(Режим=0) Экспорт
  ...
  COM_УдалитьИнформационнуюБазу(Режим);
...
//
Функция ЗапуститьПредприятие(ПутьКОбработке = "", ОжидатьЗавершения = Ложь, ДопПараметрыЗапуска = "") Экспорт
...
Функция ОтменитьНазначениеГлавногоУзлаРИБ() Экспорт
	...
	Если ПараметрыЗапуска.ТипБазы = "S" Тогда
		СтрокаСоединения = "Srvr='" + ПараметрыЗапуска.АдресКластера + ":" + ПараметрыЗапуска.ПортКластера + "'; Ref='" + ПараметрыЗапуска.ИмяБазы + "';";
	Иначе
		СтрокаСоединения = "File='" + ПараметрыЗапуска.ИмяБазы + "';";
	КонецЕсли;
	СтрокаСоединения = СтрокаСоединения + "usr='" + ПараметрыЗапуска.ИмяПользователя + "'; pwd='" + ПараметрыЗапуска.ПарольПользователя + "';";	
